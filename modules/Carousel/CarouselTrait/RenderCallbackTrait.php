...
trait RenderCallbackTrait {
	...
	public static function render_callback( $attrs, $content, $block, $elements ) {
		...
		$show_title       = self::get_attribute_value( $attrs, 'showTitle', 'on' );
		$show_image       = self::get_attribute_value( $attrs, 'showImage', 'on' );
		$gallery_ids      = self::get_attribute_value( $attrs, 'galleryIds', '' );
		...
		if ( ! empty( $gallery_ids ) ) {
			...
			foreach ( $ids as $id ) {
				...
				$slide_image_html = '';
				if ( 'on' === $show_image && ! empty( $image_url ) ) {
					$slide_image_html = HTMLUtility::render(
						[
							'tag'               => 'div',
							'attributes'        => [
				...
				$slide_title_html = '';
				if ( 'on' === $show_title && ! empty( $title ) ) {
					$slide_title_html = HTMLUtility::render(
						[
							'tag'               => 'div',
							'attributes'        => [
							...
							'children'          => HTMLUtility::render(
								[
									'tag'        => 'h3',
									'attributes' => [
										'class' => 'cg_carousel_item__title',
									],
									'children'   => esc_html( $title ),
								]
							),
						]
					);
				...
				$dynamic_content .= HTMLUtility::render(
					[
						'tag'               => 'div',
						'attributes'        => [
							'class' => 'cg_carousel_item',
						],
						'childrenSanitizer' => 'et_core_esc_previously',
						'children'          => HTMLUtility::render(
							[
								...
								'childrenSanitizer' => 'et_core_esc_previously',
								'children'          => $slide_image_html . $slide_title_html,
							]
						),
					]
				);
